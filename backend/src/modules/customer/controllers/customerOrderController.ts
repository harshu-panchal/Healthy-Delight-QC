...
export const createOrder = async (req: Request, res: Response) => {
    ...
    try {
        ...
        if (!isScheduled && (!timeSlot || (typeof timeSlot === 'string' && timeSlot.trim() === ''))) {
            if (session) await session.abortTransaction();
            return res.status(400).json({
                success: false,
        ...
        if (isScheduled) {
            ...
            const startOfScheduled = new Date(parsedDate);
        ...
        for (const item of items) {
            ...
            // Populate product tax reference recursively
            if (product.tax) {
            ...
            // Calculate item tax
            let itemTax = 0;
            if (product.tax && typeof product.tax === 'object' && 'percentage' in product.tax) {
                const taxPercentage = (product.tax as any).percentage || 0;
                itemTax = (itemPrice * (taxPercentage / 100)) * qty;
        ...
        try {
            ...
            if (io && (newOrder.status as string) !== 'Pending') {
                ...
                if (savedOrder) {
                    const eventType = savedOrder.orderType === 'Scheduled' ? 'NEW_SCHEDULED_ORDER' : 'NEW_ORDER';
                    await notifySellersOfOrderUpdate(io, savedOrder, eventType);
                }
            }
...
export const getMyOrders = async (req: Request, res: Response) => {
    try {
        const userId = req.user!.userId;
        const { status, orderType, scheduledDate, page = 1, limit = 10 } = req.query;
        ...
        if (scheduledDate) {
            ...
            const endOfScheduled = new Date(parsedDate);
            endOfScheduled.setHours(23, 59, 59, 999);
            query.scheduledDate = { $gte: startOfScheduled, $lte: endOfScheduled };
        }
...
export const getOrderById = async (req: Request, res: Response) => {
    try {
        ...
        const transformedOrder = {
            ...
            // Include customer's permanent delivery OTP if rider accepted OR if order is out for delivery/active
            deliveryOtp: (
                (order.deliveryBoy && order.deliveryBoyStatus !== 'Pending') ||
                ['Out for Delivery', 'Picked up', 'Picked Up', 'On the way', 'In Transit'].includes(order.status)
...
// Update items of an existing scheduled order
export const updateScheduledOrderItems = async (req: Request, res: Response) => {
    let session: mongoose.ClientSession | null = null;
    try {
        try {
            session = await mongoose.startSession();
        ...
        const { id } = req.params;
        const { items, scheduledDate, scheduledTimeSlot } = req.body;
        const userId = req.user!.userId;
        ...
        if (order.orderType !== 'Scheduled') {
            if (session) await session.abortTransaction();
            return res.status(400).json({ success: false, message: "Only scheduled orders can be modified this way" });
        }

        if (!['Scheduled', 'Accepted'].includes(order.status)) {
            if (session) await session.abortTransaction();
            return res.status(400).json({
                success: false,
                message: `Cannot modify items for an order in status: ${order.status}`
            });
        }

        const oldOrderItems = order.items as any[];
        for (const oldItem of oldOrderItems) {
            const qty = oldItem.quantity || 0;
            if (oldItem.variation) {
                await Product.findOneAndUpdate(
                    {
                        _id: oldItem.product,
                        $or: [
                            { "variations._id": mongoose.isValidObjectId(oldItem.variation) ? oldItem.variation : new mongoose.Types.ObjectId() },
                            { "variations.value": oldItem.variation },
                            { "variations.title": oldItem.variation },
                            { "variations.pack": oldItem.variation }
                        ]
                    },
                    { $inc: { "variations.$.stock": qty, stock: qty } },
                    { session }
                ...
                const checkProduct = await Product.findById(oldItem.product);
                if (checkProduct && checkProduct.variations && checkProduct.variations.length > 0) {
                    await Product.findOneAndUpdate(
                        { _id: oldItem.product },
                        { $inc: { "variations.0.stock": qty, stock: qty } },
        ...
        const oldItemIds = oldOrderItems.map(item => item._id);
        await OrderItem.deleteMany({ _id: { $in: oldItemIds } }, { session });
        ...
        for (const item of items) {
            ...
            newOrderItemIds.push(newOrderItem._id as mongoose.Types.ObjectId);
        }

        if (sellerIds.size > 0 && order.deliveryAddress?.latitude != null && order.deliveryAddress?.longitude != null) {
            const uniqueSellerIds = Array.from(sellerIds).map(id => new mongoose.Types.ObjectId(id));
            const sellers = await Seller.find({
                _id: { $in: uniqueSellerIds },
                status: "Approved",
            ...
            for (const seller of sellers) {
                if (seller.location && seller.location.coordinates) {
                    ...
                    const serviceRadius = seller.serviceRadiusKm || 10;
                    if (distance > serviceRadius) {
                        throw new Error(`Delivery address is out of range (${distance.toFixed(2)} km) for seller: ${seller.storeName}`);
                    }
                }
            }
        }
        ...
        order.subtotal = Number(calculatedSubtotal.toFixed(2));
        order.total = Number(finalTotal.toFixed(2));
        ...
        try {
            ...
            if (io) {
                ...
                if (savedOrder) {
                    await notifySellersOfOrderUpdate(io, savedOrder, 'NEW_SCHEDULED_ORDER');
                }
            }
        } catch (notificationError) {
            console.error("Error notifying seller of modified scheduled order:", notificationError);
        }
        ...
        return res.status(500).json({
            success: false,
            message: error.message || "Failed to update scheduled order items",
            error: error.message
        });
    } finally {
        if (session) session.endSession();
