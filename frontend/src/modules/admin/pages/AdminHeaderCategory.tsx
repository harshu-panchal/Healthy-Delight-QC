...
export default function AdminHeaderCategory() {
  ...
  const [selectedCategory, setSelectedCategory] = useState(""); // (kept for legacy search/filter)
  const [selectedTheme, setSelectedTheme] = useState("all"); // This maps to themeKey (color)
  ...
  const filteredIcons = useMemo(() => {
    ...
    // Dairy-only icon set (hide unrelated icons)
    const DAIRY_ICON_NAMES = new Set([
      "grocery-basket", // general dairy/grocery
      "ice-cream", // desserts
      "coffee-tea", // beverages
      "bakery", // bread/cream/butter vibe
      "milk-bottle",
      "cheese",
      "yogurt-cup",
      "butter",
    ...
    const dairyIcons = ICON_LIBRARY.filter((i) => DAIRY_ICON_NAMES.has(i.name));

    if (!term.trim()) return dairyIcons;

    const lowerTerm = term.toLowerCase();

    return [...dairyIcons].sort((a, b) => {
  ...
  const filteredCategories = headerCategories.filter(
    (category) =>
  ...
  const handleAddOrUpdate = async () => {
    ...
    try {
      // Generate a slug from the header category name for uniqueness
      const baseSlug = headerCategoryName
        .trim()
        .toLowerCase()
        .replace(/[^a-z0-9]+/g, "-")
        .replace(/(^-|-$)/g, "") || "header-category";

      const payload = {
        name: headerCategoryName,
        iconLibrary: selectedIconLibrary,
        iconName: headerCategoryIcon,
        // Use name-based slug as unique identifier
        // When editing, we don't override existing slug
        slug: editingId ? undefined : baseSlug,
        themeKey: selectedTheme, // store chosen color separately
  ...
  const handleEdit = (category: HeaderCategory) => {
    ...
    // When editing, restore the saved themeKey if available, else fall back to slug
    setSelectedTheme((category as any).themeKey || category.slug || "all");
  ...
  const handleDelete = async (id: string) => {
    if (
      window.confirm("Are you sure you want to delete this header category?")
    ) {
      try {
            ...
            <div>
              ...
              <div className="grid grid-cols-4 gap-3 bg-neutral-50 p-3 rounded border border-neutral-200">
                {themeOptions.map((themeKey) => {
                  ...
                  const displayColor =
                    THEME_COLOR_LABELS[themeKey] || themeKey;
                    ...
                    <tr
                      ...
                      <td className="px-4 py-3 text-sm text-neutral-600">
                        <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-neutral-100 text-neutral-800 capitalize border border-neutral-200">
                          {(() => {
                            const themeKey =
                              (category as any).themeKey || category.slug || "all";
                            const color =
                              themes[themeKey]?.primary?.[0] ||
                              themes.all.primary[0];
                            const label =
                              THEME_COLOR_LABELS[themeKey] || themeKey;
                            return (
                              <>
