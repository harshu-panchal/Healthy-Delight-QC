...
export default function DashboardCard({ icon, title, value, accentColor, onClick }: DashboardCardProps) {
  ...
  return (
    <div 
      onClick={onClick}
      className={`bg-white rounded-lg shadow-sm border border-neutral-200 p-3 sm:p-4 md:p-5 transition-all duration-200 ${
        isClickable 
          ? 'cursor-pointer hover:shadow-md hover:-translate-y-0.5 hover:border-neutral-300 active:scale-[0.98]' 
          : ''
      }`}
    >
